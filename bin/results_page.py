...
def render_results_page(get_run_paths_fn):
    ...
    # ── Sélecteur de run ──────────────────────
    # Dériver runs_base depuis get_run_paths_fn (chemin dynamique, pas hardcodé)
    runs_base = get_run_paths_fn("__probe__")["base"].parent
    available = sorted(
        ...
        if st.button("📥 Générer le PDF", key="gen_pdf", width="stretch"):
            with st.spinner("Génération du PDF en cours..."):
                try:
                    ...
                    logo = runs_base.parent / "images" / "logoCIGASS.png"
